...
static napi_value CreateNapiString(napi_env env, const char *c_str) {
    ...
    if (status != napi_ok) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG, "CreateNapiString: 转换字符串失败，错误码：{public}%d",
                     status);
        return nullptr;
    }
...
static napi_value ConnectCamera(napi_env env, napi_callback_info info) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "ConnectCamera: 型号名：%s，路径：%s", camera_name,
                 camera_path);
    ...
    if (port_index < 0) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG, "ConnectCamera: 未找到相机端口，路径：%s，索引：%d",
...
static napi_value SetCameraParameter(napi_env env, napi_callback_info info) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "SetCameraParameter: 参数名：{public}%s，参数值：{public}%s",
    ...
    if (!CheckGpError("gp_widget_get_child_by_name", ret) || target_widget == nullptr) {
        ...
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG, "SetCameraParameter: 相机不支持该参数：{public}%s",
                     param_name);
        return nullptr;
    }
...
static napi_value TakePhoto(napi_env env, napi_callback_info info) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "TakePhoto: 拍照成功！照片路径：{public}%s/{public}%s",
...
static napi_value GetPreview(napi_env env, napi_callback_info info) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "GetPreview: 获取预览成功，数据长度：{public}%lu 字节",
                 data_len);
...
static napi_value DownloadPhoto(napi_env env, napi_callback_info info) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "DownloadPhoto: 待下载照片路径：{public}%s/{public}%s",
    ...
    int ret = gp_camera_file_get(g_camera,            // 已连接的相机对象
                                 photo_folder,        // 照片文件夹
                                 photo_name,          // 照片文件名
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "DownloadPhoto: 照片下载成功，数据长度：{public}%lu 字节",
                 data_len);
